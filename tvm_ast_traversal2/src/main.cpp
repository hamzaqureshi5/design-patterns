...
struct Expr : ObjectRef {
    ...
    void print() const { get()->print(); }
    double eval(std::unordered_map<std::string,double>& env) const { return get()->eval(env); }
};
...
struct Stmt : ObjectRef {
    ...
    void print() const { get()->print(); }
    void exec(std::unordered_map<std::string,double>& env) const { get()->exec(env); }
};
...
struct IntImmNode : BaseExprNode {
    ...
    explicit IntImmNode(double v) : value(v) {}
    void print() const override { std::cout << value; }
    double eval(std::unordered_map<std::string,double>&) const override { return value; }
};
...
struct VarNode : BaseExprNode {
    ...
    void print() const override { std::cout << name; }
    double eval(std::unordered_map<std::string,double>& env) const override {
        if (!env.count(name)) throw std::runtime_error("Undefined var: " + name);
        return env[name];
    }
};
...
struct BinaryOpNode : BaseExprNode {
    std::string op;
    Expr a, b;
    BinaryOpNode(std::string o, Expr l, Expr r) : op(std::move(o)), a(std::move(l)), b(std::move(r)) {}
    void print() const override {
        std::cout << "("; a.print(); std::cout << " " << op << " "; b.print(); std::cout << ")";
    }
    double eval(std::unordered_map<std::string,double>& env) const override {
        double x = a.eval(env), y = b.eval(env);
        if (op == "+") return x + y;
...
struct LetStmtNode : BaseStmtNode {
    std::string var;
    Expr value;
    LetStmtNode(std::string v, Expr val) : var(std::move(v)), value(std::move(val)) {}
    void print() const override {
        std::cout << "let " << var << " = "; value.print(); std::cout << ";" << std::endl;
    }
...
struct EvaluateNode : BaseStmtNode {
    ...
    void print() const override {
        std::cout << "evaluate("; expr.print(); std::cout << ");" << std::endl;
    }
    void exec(std::unordered_map<std::string,double>& env) const override {
...
// ====== Helper Builders ======
inline Expr IntImm(double v) { return Expr(std::make_shared<IntImmNode>(v)); }
inline Expr Var(const std::string& n) { return Expr(std::make_shared<VarNode>(n)); }
inline Expr Add(Expr a, Expr b) { return Expr(std::make_shared<BinaryOpNode>("+", a, b)); }
inline Expr Sub(Expr a, Expr b) { return Expr(std::make_shared<BinaryOpNode>("-", a, b)); }
inline Expr Mul(Expr a, Expr b) { return Expr(std::make_shared<BinaryOpNode>("*", a, b)); }
inline Expr Div(Expr a, Expr b) { return Expr(std::make_shared<BinaryOpNode>("/", a, b)); }
...
inline Stmt LetStmt(const std::string& v, Expr e) { return Stmt(std::make_shared<LetStmtNode>(v, e)); }
inline Stmt Evaluate(Expr e) { return Stmt(std::make_shared<EvaluateNode>(e)); }
inline Stmt Seq(const std::vector<Stmt>& s) { return Stmt(std::make_shared<SeqStmtNode>(s)); }
inline Stmt IfThenElse(Expr cond, Stmt t, Stmt e) { return Stmt(std::make_shared<IfThenElseNode>(cond, t, e)); }
